...
*** [[https://archive.org/details/galaxymagazine][Galaxy Magazine]] :Comic:SciFi:Art:
Galaxy Science Fiction was an American digest-size science fiction magazine,
published from 1950 to 1980. It was founded by an Italian company, World Editions,
which was looking to break in to the American market. World Editions hired as editor
H. L. Gold, who rapidly made Galaxy the leading science fiction (sf) magazine
of its time, focusing on stories about social issues rather than technology. 
* competitors
...
*** [[https://jax-ml.github.io/scaling-book/][Scaling LLM]] :LLM:Scale:
*** [[https://damek.github.io/random/basic-facts-about-gpus/][Facts about GPU]] :GPU:Hardware:
I’ve been trying to get a better sense of how GPUs work.
I’ve read a lot online, but the following posts were particularly helpful:
** FP General
...
*** Futhark
**** [[https://futhark-lang.org/blog/2018-09-21-futhark-0.7.1-released.html#histogram-computations][Futhark Blog on Histograms]] :Futhark:Programming:
Interesting article on the difficulty of parallelizing bin sorting problems in a functional context.
...
** database related
*** [[https://spacetimedb.com/pricing][spacetimedb]] :db:multiplayer:
Today has been a very long time coming and I'm so excited that we're finally here. I'm incredibly thrilled to finally announce the release of SpacetimeDB 1.0!

What initially started as crude dream to improve the development process for a social media app, and became the inspiration for a completely new way of
developing multiplayer games when we began working on our MMORPG, BitCraft Online, has culminated in a production ready, real-time, relational database
sync engines for games and multiplayer applications.

SpacetimeDB 1.0 is the product of the collective efforts of some amazingly talented engineers over years to deliver a stable, powerful relational
database system, with elegant and easy-to-use APIs implemented in three different programming languages. Multiple teams already run SpacetimeDB
in production, and we have been testing it for years as the backend of BitCraft Online.

*** [[https://umbra-db.com/][kUMBRA]]                             :db:column:
Made by the team that built Tableau.
From the Analytics Engineering Podcast.
...
*** [[https://github.com/janestreet/incremental][incremental parallel]] :incremental:distributed:ComputerScience:paper:Ocaml:
Incremental is a library that gives you a way of building complex computations that can update efficiently in
response to their inputs changing, inspired by the work of Umut Acar et. al. on self-adjusting computations.
...
**** [[https://corrode.dev/blog/defensive-programming/][Defensive Programming in Rust]] :rust:fp:
**** [[https://rust-unofficial.github.io/too-many-lists/][Learn Rust with too many linked lists]] :rust:fp:
I fairly frequently get asked how to implement a linked list in Rust.
The answer honestly depends on what your requirements are, and it's obviously
not super easy to answer the question on the spot. As such I've decided to
write this book to comprehensively answer the question once and for all.
...
** Case Study
*** [[https://renerocks.ai/blog/2025-11-02--tigerfans/][High Performance Ticketing]] :Ticketing:EventLog:HighPerformance:CaseStudy:
...
* Developer Tools   
** [[https://statecharts.dev/][State Charts]] :Visualization:
Put simply, a statechart is a beefed up state machine.
The beefing up solves a lot of the problems that state machines have,
especially state explosion that happens as state machines grow.
...
* History
** [[https://www.oldmapsonline.org/en/history/rulers#position=2.436/43.9/48.9/-5.41&year=1631][World Map Over Time]] :History:Map:
Timeline with map of history of the world
...
** [[https://towardsdatascience.com/the-economics-of-two-sided-platforms/][Economics of 2 sided platforms]] :econometrics:prediction:
Data science as a field is great at measurement and algorithms,
but falls behind in problem formulation (which we have left to PMs and execs).
Understanding these theoretical foundations informs how we think about what metrics
to measure and what algorithms to build. 
...
*** [[https://promwad.com/news/webassembly-for-embedded-systems-cross-platform-firmware][Cross Platform Web assembly]] :Embedded:WebAssembly:
** rf theory
...
* Machine Learning
** [[https://magazine.sebastianraschka.com/p/llm-evaluation-4-approaches][Building Reasoning from Scratch]] :MachineLearning:Reasoning:
** [[https://karpathy.github.io/2019/04/25/recipe/][Andrej Karpathy's incredibly useful guide to model training]] :MachineLearning:NN:Stats:
This is an amazing set of steps to understand Machine learning.
So I thought it could be fun to brush off my dusty blog to expand my tweet to the long form that this topic deserves.
However, instead of going into an enumeration of more common errors or fleshing them out, I wanted to dig a
bit deeper and talk about how one can avoid making these errors altogether (or fix them very fast). The trick to
doing so is to follow a certain process, which as far as I can tell is not very often documented.
Let’s start with two important observations that motivate it.
** [[https://arxiv.org/pdf/2404.17625][AAlice’s adventures in a differentiable wonderland]] :MachineLearning:NN:AutomatedDifferentiation:
...
** [[https://www.argmin.net/p/reformist-reinforcement-learning][Reformed Reinforcement Learning]] :MachineLearning:ReinforcementLearning:
This means that if you can sample from p(x; w) for any setting of the weights, you can run
stochastic gradient ascent on the objective you care about. That’s it. For historical reasons,
stochastic gradient with this gradient estimator is called policy gradient. You choose a distribution
parameterized so you can sample from it. You draw samples from your current distribution.
You tune the weights to increase the probability where the samples found promising values.
** [[http://incompleteideas.net/book/RLbook2020.pdf][Reinforcement Learning]] :MachineLearning:ReinforcementLearning:
** [[https://towardsdatascience.com/temporal-difference-learning-and-the-importance-of-exploration-an-illustrated-guide/][Temporal Difference Learning]] :MachineLearning:ReinforcementLearning:
...
** [[https://damek.github.io/random/basic-facts-about-gpus/][GPU Basic Facts 2025]] :MachineLearning:
Table of Contents
...
** [[https://towardsdatascience.com/the-greedy-boruta-algorithm-faster-feature-selection-without-sacrificing-recall/][Feature Selection Algorithm]]:FeatureSelction:
The Greedy Boruta Algorithm: Faster Feature Selection Without Sacrificing Recall
Feature selection remains one of the most critical yet computationally expensive
steps in the machine learning pipeline. When working with high-dimensional datasets,
identifying which features truly contribute to predictive power can mean the
difference between an interpretable, efficient model and an overfit, sluggish one.
...
* Prompting and Vibe Stuff
** [[https://towardsdatascience.com/prompt-engineering-for-time-series-analysis-with-large-language-models/][Prompt Engineering for Time-Series Analysis with Large Language Models]] :PromptEngineering:TimeSeries:
Working with time-series data is often different from regular analysis, mainly because of challenges
regarding the time-dependency that every data scientist eventually runs into.
...
* management & business
** [[https://lawsofsoftwareengineering.com/][Laws of Software Engineering]]
...
* math
** John Cook Interesting Articles
*** [[https://www.johndcook.com/blog/2024/09/07/aryabhata/][Cosine Approximation]]
...
*** [[https://www.johndcook.com/blog/2024/08/31/craigs-formula/][Normal Integral approx]]
If you wanted to compute this probability numerically, you could obviously evaluate its defining
integral numerically. But as is often the case in numerical analysis, the most obvious
approach is not the best approach. The range of integration is unbounded and it varies with the argument.
*** [[https://www.johndcook.com/blog/2024/08/18/bessel-everett/][Bessel Everett Interpolation]]
I never heard of Bessel or Everett interpolation until long after college. I saw Lagrange interpolation several times.
Why Lagrange and not Bessel or Everett?

First of all, Bessel interpolation and Everett interpolation are not different kinds of interpolation; they
are different algorithms for carrying out the same interpolation as Lagrange. There is a unique polynomial
of degree n fitting a function at n + 1 points, and all three methods evaluate this same polynomial.
*** [[https://www.johndcook.com/blog/2024/08/15/distorted-cosines/][Distorted Cosines]]
Another gem from Numerical Methods that work.
...
** probability
*** [[https://www.perfectlynormal.co.uk/blog-kl-divergence][KL Divergence Explained]] :probability:InformationTheory:KLDivergence:
A clear, visual walkthrough of Kullback-Leibler divergence: building it up from
information/surprise and entropy, showing how it measures the extra cost of using
one probability distribution to approximate another, and why it is asymmetric.
*** [[https://win-vector.com/2024/12/19/kelly-cant-fail/][Kelly Can't Fail]] :KellyCriterion:betting:probability:
This was a really interesting article on a game to illustrate kelly betting.
The final result was the standard binary transmission with rate p of error and q of success.
p=q don't bet
p>q bet on p
p<q bet on q
*** [[https://www.sciencedirect.com/topics/mathematics/borel-field][Borel Field In Depth]] :math:search:probability:   
...
*** [[https://arxiv.org/abs/2001.10488][Statistical Consequences of Fat Tails]] :probability:stat:FatTails:Taleb:
(bayesian model checking)
A Gaussian distribution 10 𝜎 event is ~ 1/10^23
A [[https://en.wikipedia.org/wiki/Student%27s_t-distribution][Student T]] distribution 10 𝜎 event is ~ 1/200

Taleb then computes the probability that, given a 10 𝜎 event happened, the distribution was Gaussian.

P(Gaussian) = 0.999999
P(Gaussian|ItHappened) = P(G)·P(Event|G) / P(ItHappened)
Use [[https://en.wikipedia.org/wiki/Law_of_total_probability][Law of total probability]] to transform:
P(Gaussian|ItHappened) = P(G)·P(Event|G) / [(1 − P(Gaussian))P(Event|NonGaussian) + P(Gaussian)P(Event|Gaussian)]
Computing we get the following even in the case that the probability it is gaussian is high.
P(Gaussian|ItHappened) = 2 × 10⁻¹⁵
...
*** [[https://www.tangramvision.com/blog/the-deceptively-asymmetric-unit-sphere][Unit Sphere]]

+ File on disk [[file:~/notes/blog-posts/diff-geo-articles/tanagram-work.org][Diff Geo Notes]]
+ Part I: Focal Length And Collinearity
+ Part 2: Introducing Lens Distortion
+ Part 3: Exploring Distortion and Distortion Models
+ Part 4: Pinhole Obsession
+ Part 5: The Deceptively Asymmetric Unit Sphere

**** [[https://math.stackexchange.com/questions/3766220/what-is-exponential-map-in-differential-geometry][Exponential Map]] :geometry:differentialgeometry:
Is expq(v) a projection of point q to some point q′ along the geodesic
whose tangent (right?) at q is the vector v? And so expq(v) is the projection
of point q to some point along the geodesic between q and q′?
**** [[https://www.cs.cmu.edu/~kmcrane/projects/dgpdec/paper.pdf][discrete differential geometry]]           :geometry:differentialgeometry:
**** [[https://people.math.ethz.ch/~salamon/PREPRINTS/diffgeo.pdf][What is Differrential Geometry]]           :geometry:differentialgeometry:
...
* performance related
** Emacs
*** [[https://coredumped.dev/2025/06/18/making-tramp-go-brrrr./][Emacs Tramp Performance]] :optimization:emacs:
Same topic, but the [[https://news.ycombinator.com/item?id=44356346][HN article]]. 
...
* productivity
** [[https://docs.google.com/document/d/1ZJZbv4J6FZ8Dnb0JuMhJxTnwl-dwqx5xl0s65DE3wO8/edit][CEO within]] :Business:Productivity:
A document with loads of interesting CEO stuff
...
* Reverse Engineering
** [[https://x86re.com/][Reverse Engineering Blog]] :ReverseEngineering:
At a high level, reverse engineering is learning how something works
without knowledge of it. With software, our goal is to understand the
source code of a binary which we do not have the source code to. As a
question: 'what does this do, and how does it do it?' Our desired
outcomes come in varying degrees, from essentially re-creating software
to just understanding what certain functions are performing. A large
amount of analysis can be done by simply running and observing software,
however this document is primarily about static analysis and examining
the executable machine code. We gain a detailed view of the binary
because, simply put, we are looking at the exact instructions which are
running on a CPU.
...
* security related
** [[https://corrode.dev/blog/bugs-rust-wont-catch/][Bugs Rust won't catch]] :CVE:Security:Rust:
What’s notable is that all of these bugs landed in a production Rust codebase,
written by people who knew what they were doing, and none of them were caught
by the borrow checker, clippy lints, or cargo audit.

I’m not writing this to criticize the uutils team. Quite the contrary; I actually
want to thank them for sharing the audit results in such detail so that we can all learn from them.

We also had Jon Seager, VP Engineering for Ubuntu, on our ‘Rust in Production’ podcast
recently and a lot of listeners appreciated his honesty about the state of Rust at Canonical.

If you write systems code in Rust, this is the most concentrated look at where Rust’s
safety ends that you’ll likely find anywhere right nowo.

** [[https://tailscale.com/blog/managing-passwords-is-so-2020][Tailscale Password Advisor]] :security:
Using open standards to govern security behavior.
...
** [[https://wikigrapher.com/paths][Wikigraph]] :Wikipedia:graphtheory:
You can take any two subjects and graph them together in wiki form. 
* ux ui api dsl 
...
** [[https://float.exposed/0x44bf9400][All Floating Point variant analyzer]] :Numeric:Double:IEEE:
* Writing
** [[https://refactoringenglish.com/chapters/rules-for-software-tutorials/][Writing Software Tutorials]]
0    Write for beginners
1    Promise a clear outcome in the title
2    Explain the goal in the introduction
3    Show the end result
4    Make code snippets copy/pasteable
5    Use long versions of command-line flags
6    Separate user-defined values from reusable logic
7    Spare the reader from mindless tasks
8    Let computers evaluate conditional logic
...
** [[https://refactoringenglish.com/chapters/release-announcements/][Writing Press Releases]]
Key Takeaways:
A a release announcement explains how the users experience is better today than yesterday.
Release notes are not the same as release announcements and should be treated differently.

A few questions:
What can a user do they couldn't before?
What workflows became faster?
What workflows became easier?

I guess these are also important things to think about when designing features!

Focus on the positive:
"Faster" not "less slow".

No  generic terms like "various improvements"

* Game Design
** [[https://book.leveldesignbook.com/][Level Design Book]] :GameDesign:Levels:
The Level Design Book gathers level design knowledge for 3D video games in an approachable,
up-to-date, and critical way. It is for designers of all experience levels and game engines.

** [[https://www.cs.cmu.edu/~112/notes/student-tp-guides/Terrain.pdf][Terrain Generation Techniques]]
Guide to several algorithms for terrain generation and other references

** [[https://google.github.io/filament/Materials.md.html][Guide for game materials]] :GameDesign:Graphics:
Filament is a physically based rendering (PBR) engine for Android.
Filament offers a customizable material system that you can use to create
both simple and complex materials. This document describes all the features
available to materials and how to create your own material. 
